...
@Results(value={
	@Result(name="report", location="/admin/Report.jsp")
})
public class ReportAction extends SecurityAction{
	...
	public void reportByProductByExcel() throws Exception {
		title = "Hàng hóa";
		hql = "SELECT product.name, SUM(unitPrice*quantity*(1-discount))"
				+" FROM OrderDetail "
	...
	public String reportByCategory() throws Exception {
		title = "Chủng loại";
		hql = "SELECT product.category.name, SUM(unitPrice*quantity*(1-discount))"
				+" FROM OrderDetail "
				+" WHERE order.orderDate BETWEEN :min AND :max "
